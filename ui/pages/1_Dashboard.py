...
- **20-day ATR:** `${pm['atr_20']:.2f}` (volatility)
...
""")
        if pm["filter_c_active"]:
            st.caption("⚠️ Filter C active — TP1 reduced to 50% (low-volatility day)")
        if pm["long_filter_f"] or pm["short_filter_f"]:
            st.caption("✅ Filter F active — TP2 extended to 127% on "
                        f"{'long' if pm['long_filter_f'] else 'short'} side")
...
| Take profit 2 | `${pm['long_tp2']:.2f}` | `${pm['short_tp2']:.2f}` |
""")
...
if snap.position is None:
    ...
    if p["accelerated_tp1"]:
        st.caption("⏰ TP1 was accelerated to 80% of original distance at 15:30 NY")
...
gcol[3].metric("Wrong trend", w.get("gate_block_trend", 0))
gcol[4].metric("Already traded", w.get("gate_block_daily_lock", 0))
